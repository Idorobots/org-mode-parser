...
def load_raw(path: str | Path) -> tree_sitter.Tree:
    ...
    The returned :class:`~tree_sitter.Tree` gives full access to the parse
    tree via its :attr:`~tree_sitter.Tree.root_node` attribute. No
    ...
    Returns:
        A :class:`~tree_sitter.Tree` whose root node has type ``"document"``.
    ...
    Example::
        ...
        tree = load_raw("notes.org")
        root = tree.root_node
        print(root.type)  # "document"
        print(root.child_count)  # number of top-level sections / headings
    """
    resolved = Path(path)
